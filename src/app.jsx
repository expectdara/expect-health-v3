...
function Intake({onDone,mainRef,initialEmail}){
  ...
  const startMicCheck=async()=>{try{const stream=await navigator.mediaDevices.getUserMedia({audio:true});techStreamRef.current=stream;const ctx=new(window.AudioContext||window.webkitAudioContext)();techAudioRef.current=ctx;const src=ctx.createMediaStreamSource(stream);const analyser=ctx.createAnalyser();analyser.fftSize=256;src.connect(analyser);const buf=new Uint8Array(analyser.frequencyBinCount);let detected=false;const loop=()=>{analyser.getByteFrequencyData(buf);const avg=buf.reduce((a,b)=>a+b,0)/buf.length;setMicLevel(Math.min(avg/60,1));if(avg>5&&!detected){detected=true;setMicOk(true)}techAnimRef.current=requestAnimationFrame(loop)};loop();const t0=performance.now();try{await fetch(window.location.origin,{method:"HEAD",cache:"no-store"});setNetMs(Math.round(performance.now()-t0))}catch(e){setNetMs(-1)}}catch(e){setNetMs(-1)}};
