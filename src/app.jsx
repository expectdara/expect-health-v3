...
function Intake({onDone,mainRef,initialEmail,partnerRef}){
  ...
  const initAns=(()=>{const base=initState?initState.ans:(()=>{const a={};if(initialEmail)a.email=initialEmail;if(partnerRef)a.partner=partnerRef;return a})();
    // Safety: strip any plain objects from restored ans (prevents React error #300)
    const clean={};for(const[k,v]of Object.entries(base)){if(v!==null&&v!==undefined&&typeof v==="object"&&!Array.isArray(v)){console.warn("[initAns] Stripped object from key:",k);clean[k]=JSON.stringify(v)}else{clean[k]=v}}return clean})();
