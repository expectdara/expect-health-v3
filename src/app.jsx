...
const VA_CLIENT_ID=(typeof window!=="undefined"&&window.__VA_CLIENT_ID)||"0oa1aa1sgrc6Il4r62p8"; // VA Patient Health API sandbox
const VA_AUTH_URL="https://sandbox-api.va.gov/oauth2/authorization";
...
// ============================================================
// DEMO SEED DATA — Synthetic outcome records for OAIP dashboard
// ============================================================
(function seedDemoData(){
  const tiers=["Beginner","Moderate","Advanced","Foundation"];
  const subtypes=["stress","urge","mixed"];
  const ages=["18-29","30-39","40-49","50-59","60+"];
  const cues=["biologic","breathing","imaginative","simple_contract","default"];
  const exBeg=["Supine PF Activation","Diaphragmatic Breathing + PF","Bridge + PF","Endurance Kegels"];
  const exMod=["Quick-Flick Kegels","Endurance Kegels","Bridge + PF","Diaphragmatic Breathing + PF"];
  const exAdv=["Quick-Flick Kegels","Endurance Kegels","Bridge + PF","Diaphragmatic Breathing"];
  const exFnd=["Diaphragmatic Breathing + PF","Quick-Flick Kegels","Endurance Kegels","Gentle Bridge"];
  const exMap={Beginner:exBeg,Moderate:exMod,Advanced:exAdv,Foundation:exFnd};
  const dxStress=["N39.3"];const dxUrge=["N39.41"];const dxMixed=["N39.46"];
  const dxMap={stress:dxStress,urge:dxUrge,mixed:dxMixed};
  // 35 seed profiles: [iciq, pain, phq2, fsex, gupi, fluts_f, fluts_v, age_idx, subtype_idx, cue_idx, preg, popdi_pos, pudendal, avoid, med_mod, pt_mod_ex, pt_mod_adj, pt_mod_goal, rvTime, adherence, iciqD, painD, fsexD, phq2D, bowel, nps, actRes, prolapse_fu, concern]
  const seeds=[
    ...
    [5,3,0,2,9,3,2,2,"urge",2,0,0,0,1,0,0,0,0,310,89,3,1,1,0,"better",8,"yes",null,"none"],
    [2,1,0,0,3,1,1,3,"stress",3,0,0,0,0,0,0,0,0,340,97,1,0,0,0,"same",10,"yes",null,"none"],
    ...
    [0,0,0,0,1,0,0,1,"stress",2,0,0,0,0,0,0,0,0,340,95,0,0,0,0,"same",10,"yes",null,"none"],
    // Pending (outcome: null) — 2 records
  ...
  seeds.forEach((s,i)=>{
    ...
    const created=new Date(baseDate.getTime()+i*2*24*60*60*1000).toISOString();
    const pregStatus=preg==="active_pregnancy"?"active_pregnancy":preg==="postpartum"?"postpartum":"none";
    const rec={id:`DEMO-OR-${i+1}-${Date.now()+i}`,created,logicVersion:CLINICAL_LOGIC_VERSION,
      baseline:{iciq:{total:iciq,severity:iciq>=13?"Severe":iciq>=6?"Moderate":iciq>0?"Slight":"None",subtype:subtypes[typeof subtype==="number"?subtype:subtypes.indexOf(subtype)]||"stress"},fluts:{F:flutsF,V:flutsV,total:flutsF+flutsV},fsex:{total:fsex},gupi:{total:gupi,pain:Math.round(gupi*.4),urinary:Math.round(gupi*.3),qol:Math.round(gupi*.3),severity:gupi>=25?"Severe":gupi>=15?"Moderate":"Mild"},pain:{composite:pain,functional:Math.min(pain+1,10),severity:pain>=7?"Severe":pain>=4?"Moderate":"Mild"},phq2,age_bracket:ages[ageIdx]||"30-39",pregnancy_status:pregStatus,delivery_type:pregStatus==="postpartum"?"vaginal":pregStatus==="active_pregnancy"?null:null,weeks_postpartum:pregStatus==="postpartum"?12:null,constipation_composite:pain>=5,avoidance_count:avoid,cue_preference:cues[cueIdx]||"default",pudendal_flag:!!pudendal,med_modify:medMod,prior_treatment:avoid>=3?["pt"]:["none"],symptom_triggers:pudendal?["sitting_long","standing_long"]:pain>=5?["lifting","exercise"]:[],subtype:subtypes[typeof subtype==="number"?subtype:subtypes.indexOf(subtype)]||"stress",screener_pain:pain>=3,screener_sexual:fsex>=2,pelvic_history:avoid>=3?["incontinence"]:["none"],popdi:{score:popdiPos*15,positiveCount:popdiPos,bulge:popdiPos>=2,highBother:popdiPos>=2}},
      treatment:{tier,exercise_ids:exMap[tier],exercise_count:exMap[tier].length,adjunct_types:pain>=4?["pain_desensitization"]:iciq>=6?["bladder_retraining"]:[],adjunct_count:pain>=4?1:iciq>=6?1:0,cue_type:cues[cueIdx]||"default",dx_codes:[...(dxMap[subtypes[typeof subtype==="number"?subtype:subtypes.indexOf(subtype)]||"stress"]||["R32"]),...(pudendal?["G57.91"]:[]),...(popdiPos>0?["N81.9"]:[])],risk_level:phq2>=3||pain>=7||avoid>=4?"yellow":"green",prenatal_modified:pregStatus==="active_pregnancy",pt_modified_exercises:!!ptEx,pt_modified_adjuncts:!!ptAdj,pt_modified_goals:!!ptGoal,pt_rejection:false,review_time_seconds:rvTime},
...
{id:"bowel_frequency",text:"How often do you typically have a bowel movement?",opts:[["Less than once a week",0],["1–2 times per week",1],["3–4 times per week",2],["5–7 times per week",3],["1–2 times per day",4],["3 or more times per day",5]]},
...
// Urinary: same triggers as caffeine. Bowel: straining ≥ sometimes, frequency ≤ 1-2x/wk, Bristol 1-2
{id:"water_intake",text:"How many glasses of water (8 oz) do you drink per day on average?",
  conditional:a=>(a.iciq4||[]).includes("urgency")||(a.fl3a??0)>=1||(a.fl5a??0)>=2||(a.fl2a??0)>=2||(a.bowel_constipation??0)>=2||(a.bowel_frequency??4)<=1||(a.bristol_stool??4)<=2,
...
// Exclusion-to-referral mapping — reason-specific specialist routing
const EXCLUSION_REFERRALS={
  ex_mesh:{label:"Pelvic Mesh Complications",msg:"This program is not the right fit for online management alone. We recommend evaluation by a pelvic floor specialist and/or pelvic floor surgeon.",specialist:"Urogynecology / Female Pelvic Medicine & Reconstructive Surgery",providers:["uofu_urogyn"],returnEligible:true,ptAlso:true},
  ex_prolapse:{label:"Advanced Pelvic Organ Prolapse",msg:"This program is not the right fit for online management alone. We recommend evaluation by a pelvic floor specialist and/or pelvic floor surgeon.",specialist:"Urogynecology / Female Pelvic Medicine & Reconstructive Surgery",providers:["uofu_urogyn"],returnEligible:true,ptAlso:true},
  ex_neuro:{label:"Neurological Pelvic Floor Condition",msg:"Please contact your physician or treating specialist for in-person evaluation before starting pelvic floor treatment.",specialist:"Urology / Neuro-Urology",providers:["uofu_urogyn"],returnEligible:false},
  ex_cancer:{label:"Pelvic Cancer",msg:"Please contact your treating oncology team for evaluation. For gynecologic cancers, a gynecologic oncologist can coordinate your care.",specialist:"Oncology / Gynecologic Oncology",providers:["huntsman"],returnEligible:false},
  ex_infection:{label:"Active Pelvic Infection",msg:"Active pelvic infections require medical treatment before starting a pelvic floor program. Please contact your OB-GYN, primary care provider, or seek urgent medical evaluation.",specialist:"OB-GYN / Primary Care",providers:[],returnEligible:true},
...
const EXPANSION_LIB={
"biofeedback device":{n:"PF Biofeedback Device",type:"device",d:"FDA-cleared app-connected pelvic floor biofeedback sensor (Perifit Care or Perifit Care+) for real-time contraction feedback.",rx:"Consider if patient does not progress with HEP alone by week 4.",patientText:"To confirm you are isolating the correct muscles, we recommend using an FDA-cleared biofeedback device. This small sensor connects to your phone and shows you in real time when you are squeezing correctly — like a Fitbit for your pelvic floor. Research shows women who use biofeedback progress faster.\n\nRecommended devices (all FDA 510(k) cleared, available over the counter — no prescription needed):\n\n• Perifit Care (~$149) — App-connected sensor with gamified exercises. FDA 510(k) cleared for stress, urge, and mixed incontinence. Most popular option.\n• Perifit Care+ (~$179) — Upgraded dual-sensor version with more detailed contraction analysis. Same FDA clearance.\n\nBoth connect via Bluetooth to a free smartphone app that guides your exercises and tracks your progress over time. Your PT can review your data remotely.",badge:"library"},
...
const PATIENT_ADJ={
"PF Biofeedback Device":{name:"Pelvic Floor Biofeedback Device",type:"device",what:"A small sensor that connects to your phone and shows exactly when you're squeezing correctly. Like a Fitbit for your pelvic floor.",why:"Research shows women who use biofeedback progress faster.",expect:"Improved awareness within the first week. Clear strength difference by week 4.",howToGet:"These are FDA 510(k) cleared and available over the counter — no prescription needed:\n\n• Perifit Care (~$149) — Most popular. App-connected sensor with gamified exercises and progress tracking. perifit.co\n• Perifit Care+ (~$179) — Upgraded version with dual sensors for more detailed feedback.\n\nBoth connect via Bluetooth to a free smartphone app. Some insurance plans reimburse with a PT letter of medical necessity.",note:"May be recommended if exercises alone aren't enough by week 4."},
...
"Bowel Management Program":{name:"Bowel Health Program",type:"behavioral",what:"Three changes to reduce straining: (1) Use a 6-8 inch footstool on the toilet to put your knees above your hips — this straightens the anorectal angle and makes emptying easier. (2) Eat 25–30g of fiber daily (fruits, vegetables, whole grains, or a fiber supplement like psyllium husk). (3) Drink 6–8 glasses of water daily. Adding electrolytes or magnesium citrate (200-400mg at bedtime) can help soften stool.",why:"Straining puts enormous pressure on your pelvic floor — directly against the muscles you're strengthening.",expect:"Most notice easier bowel movements within 1-2 weeks.",howToGet:"Start with the footstool today — it makes the biggest immediate difference. Add fiber gradually over 1-2 weeks to avoid bloating.",note:"Use a footstool on the toilet, get 25-30g fiber daily, drink 6-8 glasses water. Adding electrolytes or magnesium citrate can help soften the stool. Never strain."},
...
function genPlan(iciq,pain,gupi,intake){
  ...
  if(iciq.subtype.includes("Stress")&&iciq.total>=10)p.adjuncts.push({type:"device",n:"PF Biofeedback Device",d:"FDA-cleared app-connected pelvic floor biofeedback sensor (Perifit Care or Perifit Care+). 510(k) cleared for stress/urge/mixed incontinence. OTC, no Rx needed.",rx:"Consider if patient does not progress with HEP alone by week 4."});
...
popdi3/popdi3_bother — bulge in vaginal area
popdi4/popdi4_bother — push to complete bowel movement
  ...
  return<div className="fi"style={{maxWidth:560,margin:"0 auto"}}>
    ...
    <div style={{marginBottom:28}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
        {symptoms.map((s,i)=><div key={i}onClick={()=>toggleSym(i)}style={{background:sel.has(i)?"rgba(76,44,132,.06)":C.g50,border:`1.5px solid ${sel.has(i)?C.purp:C.g200}`,borderRadius:10,padding:"12px 14px",fontSize:13,color:sel.has(i)?C.purp:C.g600,lineHeight:1.5,cursor:"pointer",transition:"all .15s",fontWeight:sel.has(i)?600:400,textAlign:"center"}}>
          {s}
        </div>)}
      </div>
    </div>
...
function VoiceIntake({initialAns,onBack,onDone,onSwitchToForm}){
  ...
  const finishIntake=()=>{
    ...
    const depressionFlag=phq2Total>=3?{positive:true,score:phq2Total,maxScore:6,interest:a.phq2_interest||0,mood:a.phq2_mood||0,threshold:3,recommendation:"PHQ-9 full screening recommended.",oaip_report:{flagType:"DEPRESSION_RISK",severity:phq2Total>=5?"HIGH":"MODERATE",score:phq2Total,maxScore:6,timestamp:new Date().toISOString()}}:{positive:false,score:phq2Total};
    sharedIntake={ans:a,iciq,pain,gupi,fluts,fsex,popdi,plan,depressionFlag,prenatalFlag:!!a.prenatal_flag,name:(a.name_first||"")+" "+(a.name_last||""),physicianName:a.physician_name,physicianFax:a.physician_fax,physicianNPI:a.physician_npi_id,safetyAnswerChanged:false,safetyChanges:[],userId:authSession?.userId,screeners:{pain:a.screen_pain,sexual:a.screen_sexual},pelvicHistory:a.pelvic_history||[]};
  ...
  const startCall=async()=>{
    ...
    try{
      ...
      // Normalize: strip punctuation + extra spaces for fuzzy matching
      const norm=s=>(s||"").toLowerCase().replace(/[^a-z0-9\s]/g,"").replace(/\s+/g," ").trim();
      const greetingNorm=norm(assistantConfig.firstMessage);
      let connected=false;

      // Pre-populate transcript with firstMessage (avoids phonetic STT issues with names)
      ...
      vapi.on("message",(msg)=>{
        ...
        // AI speech: skip Deepgram STT fragments that match the greeting
        ...
        if(msg.type==="transcript"&&msg.transcriptType==="final"&&msg.role==="assistant"){
          const fragNorm=norm(msg.transcript);
          if(fragNorm.length>3&&!greetingNorm.includes(fragNorm)){
  ...
  return<div className="fi"style={{maxWidth:560,margin:"0 auto"}}>
    ...
    {transcript.length>0&&<div style={{background:"#fff",border:`1px solid ${C.g200}`,borderRadius:12,maxHeight:320,overflowY:"auto",marginBottom:16}}>
      ...
      <div style={{padding:"8px 14px"}}>
        {transcript.map((t,i)=><div key={i}style={{marginBottom:8,display:"flex",gap:8,alignItems:"flex-start"}}>
          <div style={{width:24,height:24,borderRadius:"50%",background:t.role==="assistant"?C.purp:C.blue,color:"#fff",fontSize:10,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{t.role==="assistant"?"AI":"You"}</div>
          <div style={{fontSize:13,color:C.g700,lineHeight:1.5,paddingTop:2}}>{t.text}</div>
        </div>)}
    ...
    {status!=="done"&&status!=="processing"&&<div style={{textAlign:"center"}}>
      {onSwitchToForm&&<div style={{marginBottom:4}}><button className="btn"onClick={onSwitchToForm}style={{fontSize:12,color:C.g500}}>Switch to standard text form →</button></div>}
      {(status==="idle"||status==="error")&&onBack&&<button className="btn"onClick={onBack}style={{fontSize:12,color:C.g500}}>← Back</button>}
    </div>}
  </div>;
}
...
async function startVAAuth(){const cv=generateCodeVerifier();const ch=base64urlEncode(await sha256(cv));const statePayload=btoa(JSON.stringify({v:cv}));const params=new URLSearchParams({client_id:VA_CLIENT_ID,redirect_uri:VA_REDIRECT_URI,response_type:"code",state:statePayload,scope:VA_SCOPES,code_challenge:ch,code_challenge_method:"S256"});window.location.href=VA_AUTH_URL+"?"+params.toString()}
...
function Intake({onDone,mainRef,initialEmail,partnerRef}){
  ...
  const[techStep,setTechStep]=useState(0);const[micOk,setMicOk]=useState(false);const[netMs,setNetMs]=useState(null);const[speakerPlaying,setSpeakerPlaying]=useState(false);const[micErr,setMicErr]=useState("");
  const[micPhase,setMicPhase]=useState("idle");const[micDevices,setMicDevices]=useState([]);const[selectedDeviceId,setSelectedDeviceId]=useState("");const[recordSecs,setRecordSecs]=useState(0);
  ...
  useEffect(()=>{if(!vaProfile||vaApplied.current)return;vaApplied.current=true;const p=vaProfile.profile||{};console.log("[VA pre-fill] raw profile fields:",{dob:p.dob??null,sex_at_birth:p.sex_at_birth??null,email:p.email??null,phone:p.phone??null,name_first:p.name_first??null,name_last:p.name_last??null});let count=0;const fill=(k,v)=>{if(v&&!ans[k]){set(k,v);count++;console.log("[VA pre-fill] set",k,"=",v)}else if(!v){console.log("[VA pre-fill] skipped",k,"— value is",v)}else{console.log("[VA pre-fill] skipped",k,"— ans already has",ans[k])}};fill("name_first",p.name_first);fill("name_last",p.name_last);if(p.dob&&!ans.dob){const dp=p.dob.split("-");if(dp.length===3){set("dob",p.dob);set("dob_raw",dp[1]+"/"+dp[2]+"/"+dp[0]);count++;console.log("[VA pre-fill] set dob =",p.dob,"raw =",dp[1]+"/"+dp[2]+"/"+dp[0])}else{console.log("[VA pre-fill] skipped dob — unexpected format",p.dob)}}fill("sex_at_birth",p.sex_at_birth);fill("email",p.email);fill("phone",p.phone);setVaFieldCount(count);console.log("[VA pre-fill] total fields filled:",count)},[vaProfile]);
  ...
  const steps=isMale?[
    ...
    {t:"Symptom Screening",s:"These questions determine which sections of the assessment apply to you.",qs:SCREENER},
    {t:"Bladder Leakage",s:"ICIQ-UI Short Form — thinking about the past 4 weeks.",qs:[...ICIQ]},
  ...
  const qolIncomplete=step===10&&(ans.gupi7===undefined||ans.gupi8===undefined||ans.gupi9===undefined);
  ...
  if(voiceMode==="consent")return<div className="fi"style={{maxWidth:520,margin:"0 auto"}}>
    ...
    <button className="btn bpk"disabled={!voiceConsent}onClick={()=>{L("voice_consent_given",{patient:(ans.name_first||"")+" "+(ans.name_last||"")});setTechStep(0);setMicOk(false);setNetMs(null);setMicErr("");setMicPhase("idle");setSelectedDeviceId("");setVoiceMode("techcheck")}}style={{width:"100%",justifyContent:"center",opacity:voiceConsent?1:.4}}>Continue →</button>
  ...
  const cleanupMic=()=>{if(techAnimRef.current){clearInterval(techAnimRef.current);techAnimRef.current=null}if(techStreamRef.current){techStreamRef.current.getTracks().forEach(t=>t.stop());techStreamRef.current=null}if(playbackUrlRef.current){URL.revokeObjectURL(playbackUrlRef.current);playbackUrlRef.current=null}if(recorderRef.current){try{recorderRef.current.stop()}catch(e){}recorderRef.current=null}};
  return<div className="fi"style={{maxWidth:520,margin:"0 auto"}}>
    <div style={{textAlign:"center",marginBottom:20,paddingTop:8}}>
      <div className="h1"style={{fontSize:20,color:C.purp}}>Equipment Check</div>
      <div style={{fontSize:13,color:C.g500,marginTop:6,lineHeight:1.6}}>Let's make sure your audio is working before we start.</div>
    </div>
    <div style={{display:"flex",justifyContent:"center",gap:8,marginBottom:24}}>
      {["Speaker","Microphone","Ready"].map((label,i)=><div key={i}style={{display:"flex",alignItems:"center",gap:6}}>
        <div style={{width:28,height:28,borderRadius:"50%",background:techStep>i?C.gn:techStep===i?C.purp:C.g200,color:techStep>=i?"#fff":C.g500,fontSize:12,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center",transition:"all .2s"}}>{techStep>i?"✓":i+1}</div>
        <span style={{fontSize:11,fontWeight:600,color:techStep>=i?C.g700:C.g400}}>{label}</span>
        {i<2&&<div style={{width:24,height:2,background:techStep>i?C.gn:C.g200,borderRadius:1,marginLeft:2}}/>}
      </div>)}
    </div>
    {techStep===0&&<div className="card"style={{borderColor:C.purp,textAlign:"center"}}>
      <div style={{fontSize:36,marginBottom:12}}>🔊</div>
      <div style={{fontSize:15,fontWeight:700,color:C.purpD,marginBottom:8}}>Speaker Check</div>
      <div style={{fontSize:13,color:C.g600,lineHeight:1.6,marginBottom:16}}>We'll play a short tone. Make sure your volume is up.</div>
      <button className="btn bpk"onClick={playTone}disabled={speakerPlaying}style={{margin:"0 auto 16px",display:"flex",alignItems:"center",gap:8,justifyContent:"center"}}>{speakerPlaying?"Playing...":"Play Test Sound"}</button>
      <button className="btn"onClick={()=>{setTechStep(1);startMicCheck()}}style={{margin:"0 auto",display:"flex",justifyContent:"center",color:C.purp,fontSize:13,fontWeight:600}}>Yes, I can hear it →</button>
    </div>}
    {techStep===1&&<div className="card"style={{borderColor:C.purp,textAlign:"center"}}>
      ...
      {micPhase==="playing"&&<>
        <div style={{fontSize:13,color:C.g600,lineHeight:1.6,marginBottom:16}}>Playing back your recording...</div>
        <div style={{fontSize:36,marginBottom:8}}>🔊</div>
      </>}
      {micPhase==="confirm"&&<>
        <div style={{fontSize:13,color:C.g600,lineHeight:1.6,marginBottom:16}}>Did you hear your voice clearly?</div>
        <div style={{display:"flex",gap:12,justifyContent:"center",marginBottom:12}}>
          <button className="btn bpk"onClick={()=>{setMicOk(true);cleanupMic();setTechStep(2)}}style={{display:"flex",justifyContent:"center"}}>Yes, sounds great</button>
          <button className="btn bo"onClick={()=>setMicPhase("trouble")}style={{display:"flex",justifyContent:"center"}}>No, I didn't hear it</button>
        </div>
        <button className="btn"onClick={replayRecording}style={{margin:"0 auto",display:"flex",justifyContent:"center",color:C.purp,fontSize:12}}>Play again</button>
      </>}
      {micPhase==="trouble"&&<>
        <div style={{fontSize:13,color:C.g600,lineHeight:1.6,marginBottom:12}}>Let's fix that. Here are some things to try:</div>
        <div style={{textAlign:"left",background:C.g50,borderRadius:8,padding:"12px 16px",marginBottom:16,fontSize:12,color:C.g700,lineHeight:1.8}}>
          <div style={{marginBottom:4}}>Make sure your device volume is turned up</div>
          <div style={{marginBottom:4}}>Check that a headset or external mic is plugged in</div>
          <div style={{marginBottom:4}}>Close other apps using your microphone (Zoom, Teams, etc.)</div>
          <div>Try a different browser — Chrome usually works best</div>
        </div>
        {micDevices.length>1&&<div style={{marginBottom:16}}>
          <div style={{fontSize:12,fontWeight:600,color:C.g700,marginBottom:6}}>Try a different microphone:</div>
          <select value={selectedDeviceId}onChange={e=>switchDevice(e.target.value)}style={{width:"100%",padding:"10px 12px",borderRadius:8,border:`1.5px solid ${C.g200}`,fontSize:13,color:C.g700}}>
            {micDevices.map((d,i)=><option key={d.deviceId}value={d.deviceId}>{d.label||"Microphone "+(i+1)}</option>)}
          </select>
        </div>}
        <button className="btn bpk"onClick={()=>setMicPhase("idle")}style={{margin:"0 auto",display:"flex",justifyContent:"center"}}>Try recording again</button>
      </>}
      {netMs!==null&&netMs>0&&<div style={{fontSize:12,color:netMs>500?C.or:C.gn,fontWeight:600,marginTop:12}}>Network latency: {netMs}ms {netMs>500?"— voice may be slow":"— looks good"}</div>}
      {netMs===-1&&<div style={{fontSize:12,color:C.or,fontWeight:600,marginTop:12}}>Network check failed — voice may still work</div>}
      {netMs>500&&<div style={{background:"#FEF3C7",border:"1px solid #FDE68A",borderRadius:8,padding:"10px 14px",fontSize:12,color:"#78350F",lineHeight:1.6,marginTop:8}}>Your connection seems slow. Voice intake may not work well. Consider using the standard text form instead.</div>}
    </div>}
    {techStep===2&&<div className="card"style={{borderColor:C.gn,textAlign:"center"}}>
      <div style={{fontSize:36,marginBottom:12}}>✅</div>
      <div style={{fontSize:15,fontWeight:700,color:"#166534",marginBottom:8}}>You're all set!</div>
      <div style={{fontSize:13,color:C.g600,lineHeight:1.6,marginBottom:20}}>When you tap Start, our AI assistant will ask you the first question. Just speak naturally — there are no wrong answers.</div>
    ...
    <div style={{textAlign:"center",marginTop:4}}><button className="btn"onClick={()=>{cleanupMic();setTechStep(0);setMicOk(false);setNetMs(null);setMicErr("");setMicPhase("idle");setSelectedDeviceId("");setVoiceMode("consent")}}style={{fontSize:12,color:C.g500}}>← Back</button></div>
  </div>}
  ...
  return<div className="fi">
    ...
    {hasExclusion&&step===2&&triggeredExclusions.map(ex=>{const ref=EXCLUSION_REFERRALS[ex.id];if(!ref)return null;const provs=(ref.providers||[]).map(id=>UTAH_PROVIDERS.find(p=>p.id===id)).filter(Boolean);const ptProv=ref.ptAlso?UTAH_PROVIDERS.find(p=>p.type==="pt"):null;return<div key={ex.id}style={{background:"#FEF3C7",border:`1.5px solid ${C.or}`,borderRadius:10,padding:16,marginBottom:12}}>
      <div style={{fontWeight:700,fontSize:15,color:"#92400E",marginBottom:8}}>{ref.label}</div>
      <div style={{fontSize:13,color:"#78350F",lineHeight:1.6,marginBottom:12}}>{ref.msg}</div>
      <div style={{fontSize:12,color:"#92400E",fontWeight:600,marginBottom:10}}>Recommended specialist: {ref.specialist}</div>
      {provs.map(prov=><div key={prov.id}style={{background:"#fff",border:"1px solid #E5E7EB",borderRadius:8,padding:12,marginBottom:8}}>
        <div style={{fontWeight:700,fontSize:13,color:C.g800}}>{prov.name}</div>
        <div style={{fontSize:12,color:C.g500,marginBottom:6}}>{prov.cities.join(" \u00B7 ")}</div>
        <a href={"tel:"+prov.phone.replace(/[^\d]/g,"")}onClick={()=>L("exclusion_referral_call",{exclusion:ex.id,provider:prov.id,specialist:ref.specialist})}style={{display:"inline-flex",alignItems:"center",gap:6,background:C.purp,color:"#fff",padding:"8px 16px",borderRadius:8,fontSize:13,fontWeight:600,textDecoration:"none",cursor:"pointer"}}>{prov.phone}</a>
      </div>)}
      {ptProv&&<div style={{background:"#F5F3FF",border:`1px solid ${C.purpL}`,borderRadius:8,padding:12,marginTop:4,marginBottom:8}}>
        <div style={{fontSize:12,color:C.purpD,fontWeight:600,marginBottom:4}}>In-person pelvic floor PT may complement your specialist care</div>
        <div style={{fontWeight:700,fontSize:13,color:C.g800}}>{ptProv.name} \u2014 {ptProv.provider}, {ptProv.credential}</div>
        <div style={{fontSize:12,color:C.g500}}>{ptProv.cities.join(" \u00B7 ")}{ptProv.telehealth?" \u00B7 Telehealth available":""}</div>
...
function MyCareplan({data}){
  ...
  const effectiveCue=ans._assigned_cue||ans.cue_preference;
  const cueLbl={biologic:"Body function",imaginative:'Imaginative ("blueberry" cue per Crane & Dugan, 2025)',breathing:"Breath-based",simple_contract:"Simple contraction",shorten:'Shorten cue (Stafford 2016)',squeeze:'Squeeze anus (Ben Ami 2022)',urine_stop:"Stop urine flow",default:"Default"}[effectiveCue]||"Default";
  ...
  const cue=cueMap[effectiveCue]||cueMap.default;
  ...
  const ExTab=()=><div>
    <div className="cp-card">
      ...
      {effectiveCue==="biologic"&&<div className="cp-exwarn"style={{marginBottom:14}}>⚠ <strong>Important:</strong> Your exercises use the "stop the flow of urine" cue to help you find the right muscles. <strong>Never practice pelvic floor exercises during actual urination.</strong> Use the cue as a mental image only.</div>}
      {/* Cueing reminder */}
      <div style={{background:effectiveCue==="imaginative"?"#EDE9FE":effectiveCue==="breathing"?"#E0F2FE":"#FEF3C7",borderRadius:10,padding:"10px 14px",marginBottom:16,fontSize:12,lineHeight:1.5,color:effectiveCue==="imaginative"?"#4C1D95":effectiveCue==="breathing"?"#0C4A6E":"#78350F"}}>
...
function PTNewIntakeReview({data,onBack}){
  ...
  const buildNote=()=>{
    ...
    const ptEffCue=ans._assigned_cue||ans.cue_preference;
    const cueLbl={biologic:"Body function",imaginative:"Imaginative",breathing:"Breath-based",simple_contract:"Simple contraction",shorten:"Shorten cue (Stafford)",squeeze:"Squeeze anus (Ben Ami)",urine_stop:"Stop urine flow",default:"Default"}[ptEffCue]||"Default";
  ...
  // Auto-generate encounter note on mount so fax button is immediately usable
  const autoNoteRef=useRef(false);
  useEffect(()=>{if(!autoNoteRef.current){autoNoteRef.current=true;genNote()}},[]);
  ...
  const doFinalApprove=()=>{setTRun(false);setApproved(true);setShowGuardrails(false);
    const approvedPlan={...plan,status:"approved",review_flags:data?.plan?.review_flags||plan.review_flags||[],goals:editGoals.split("\n").filter(Boolean),prec:editPrec.split("\n").filter(Boolean),prog:editProg.split("\n").filter(Boolean),ex:editExs,adjuncts:editAdj};
  ...
  return<div className="fi">
    ...
    <div style={{marginBottom:14}}>
      ...
      <Section title="Demographics & History" tag={ans.pregnancy_status?.replace(/_/g," ")||""}>
        ...
        <AnsRow label="Cue preference" value={(()=>{const ec=ans._assigned_cue||ans.cue_preference;const lbl=ec==="biologic"?"Body function (stop urine flow)":ec==="imaginative"?"Imaginative (lift a blueberry)":ec==="breathing"?"Breath-based (exhale and lift)":ec==="shorten"?"Shorten cue (Stafford 2016)":ec==="squeeze"?"Squeeze anus (Ben Ami 2022)":ec==="urine_stop"?"Stop urine flow":"Default";return ans.cue_preference==="default"?lbl+" (randomly assigned — patient chose 'surprise me')":lbl})()}/>
      </Section>
...
function OAIPView(){
  ...
  const enrolled=Math.max(DPTS.length+(sharedIntake?1:0),allRecs.length);
...
function ReportIssue({pView}){
  ...
  const submit=(type)=>{
    ...
    if(type==="worsening"){L("CLINICAL_REGRESSION_FLAG",{severity,symptomType:isNew?"new":"worsening",detail:worseDesc||undefined,patient:pt});if(sharedIntake?.plan?.review_flags&&!sharedIntake.plan.review_flags.some(f=>f.id==="CLINICAL_REGRESSION_REPORTED")){sharedIntake.plan.review_flags=[...sharedIntake.plan.review_flags,{id:"CLINICAL_REGRESSION_REPORTED",type:"triggered",label:"Worsening Reported"}];notifyFlagChange()}}
    else if(type==="pain"){L("EXERCISE_PAIN_REPORT",{severity:painSeverity,context:"during_exercises",patient:pt})}
    else if(type==="adverse"){L("adverse_event_report",{category:adverseType,detail:adverseDetail,patient:pt});if(sharedIntake?.plan?.review_flags&&!sharedIntake.plan.review_flags.some(f=>f.id==="ADVERSE_EVENT")){sharedIntake.plan.review_flags=[...sharedIntake.plan.review_flags,{id:"ADVERSE_EVENT",type:"triggered",label:"Adverse Event Reported"}];notifyFlagChange()}}
